...
起因是一个很具体的问题：**被试不在同一个地方。**

传统做法是把人请到实验室来。但这意味着对方要专门腾出时间、跑一趟、迁就你的时间表——**大部分人都不愿意费这个劲**。
于是"招被试"本身成了研究里最耗人力的一环：能触达的人被地理位置锁死，样本量上不去，数据回收慢，
很多想法还没开始做，就先卡在"凑不够人"上。

这个工具想同时解决两头：

- **对被试**：什么都不用装，什么都不用注册，也不用跑一趟。收到一个文件，在电脑上双击，按提示做完就行。
- **对研究者**：能触达的人不再受地理限制，**被试量和数据回收效率**跟着上来；数据怎么回来也给三条路
  （二维码截图 / CSV / 摘要截图），哪条顺手走哪条。

顺带把两件最容易出事的事也管了：数据**不经过任何服务器**（不存在上传，也就不存在上传环节的泄漏面），
以及被试中途关页面时能自动存盘恢复。

一句话：**把"请人来实验室"的成本，降到"发一个文件"。**
